...
describe('bootstrap shell', () => {
  it('renders the product identity and the protocol version', () => {
    Object.defineProperty(window, 'layup', {
      ...
      layup: { current: vi.fn(async () => ({ youAreCreatorMembership: false })), create: vi.fn(), join: vi.fn(), leave: vi.fn(), onChanged: vi.fn(() => () => {}) },
      ui: { setMode: vi.fn(async () => ({ mode: 'home' })), onMode: vi.fn(() => () => {}) }, realtime: { status: vi.fn(async () => ({ status: 'idle', attempt: 0 })), onState: vi.fn(() => () => {}) } },
      configurable: true,
      writable: true,
    });
    render(<App />);
